...
\item{\fw{gnuplot} - Whether to use GNUplot to render equation graphs.
  Set to \fw{yes}, \fw{on}, or \fw{1} to enable.  Disabled by
  default.}
\end{itemize}
...
Also note that it is legal to have both \fw{tikz} and \fw{gnuplot}
enabled; just don't expect GNUplot to try processing code blocks for a
\fw{tikz} GNUplot preamble, since TikZ processing happens first.
...
intervals.  The most important part is the ``\fw{\#eq-basic}'' bit:
...
might create \fw{eq-preambles/eq-fancy.txt}, in which case the code
